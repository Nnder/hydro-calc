    ...
    >
      <div v-if="stepData.img" class="w-full lg:w-1/2">
        <div class="relative group cursor-pointer">
          <div
            class="absolute -inset-4 bg-gradient-to-r from-blue-100 via-blue-200 to-blue-100 rounded-2xl blur-xl opacity-60 group-hover:opacity-80 transition-all duration-700 animate-pulse-slow"
          ></div>
          <div
            class="relative overflow-hidden rounded-2xl shadow-2xl shadow-blue-500/20 border border-white/30 backdrop-blur-sm"
          >
            <NuxtImg
              :src="stepData.img"
      ...
      <div class="w-full" :class="stepData.img ? 'lg:w-1/2' : 'lg:w-full'">
        <div class="space-y-8">
          <div class="relative">
            <div
              class="absolute -left-8 top-0 w-2 h-full bg-gradient-to-b from-blue-500 to-blue-600 rounded-full"
            ></div>
            <h3
              class="text-4xl lg:text-5xl font-bold bg-gradient-to-r from-slate-800 to-blue-600 bg-clip-text text-transparent pl-4"
            >
              {{ stepData.header }}
            </h3>
          </div>
          ...
          >
            <div
              class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-blue-600 rounded-t-2xl opacity-0 group-hover:opacity-100 transition duration-500"
            ></div>

            <div
              class="prose prose-lg prose-slate max-w-none prose-p:text-slate-700 text-xl prose-p:leading-8 prose-a:text-blue-600 prose-a:no-underline hover:prose-a:underline prose-a:font-semibold prose-strong:text-slate-900 prose-strong:font-bold prose-ul:marker:text-blue-500 prose-ol:marker:text-blue-500 prose-li:marker:font-medium prose-li:leading-8 prose-headings:text-slate-900 prose-headings:font-bold prose-blockquote:border-l-blue-400 prose-blockquote:bg-blue-50/50 prose-code:text-blue-600 prose-code:bg-blue-50 prose-pre:bg-slate-900"
...
Однако давайте взглянем подробнее на хронологию этого процесса, используя в качестве примера новый проект.`,
      ...
      img: '/4862.jpg',
      endText:
        'После подтверждения заказа высококвалифицированные конструкторы начинают сбор и анализ технической документации, включая гидравлические и электрические схемы, а также каталоги запасных частей.',
    },
  },
  index: {
    type: Number,
