...
const blockDataText = {
  ...
  description: `<p class="text-lg">
          Компания ООО "АбсолютТехно" является ведущим поставщиком высококачественных уплотнительных элементов в Нижнем Тагиле. 
...
const hydrantParts = ref([
  {
    name: 'Грязесъемники',
    features: ['Для гидроцилиндров', 'Для штока', 'Для направляющих', 'Для оптических линеек'],
    description: '',
    selected: false,
    show: false,
...
`,
...

  {
    ...
    features: ['Полиуретана', 'Фторопласта', 'Полиацеталя'],
...

  {
    ...
    description:
      'Деталь в виде обруча с круглым поперечным сечением, позволяющая изолировать друг от друга две среды. Назначение колец заключается в уплотнении подвижных деталей (насосов, гидроцилиндров, поршней) и неподвижных соединений различных агрегатов и устройств: гидравлических; пневматических; смазочных; топливных. Уплотняющее воздействие достигается за счет осевого или радиального сжатия. Если статическое уплотнение для гидравлики (уплотнительное кольцо) работает некачественно, то изоляция элементов нарушается. Это приводит к выходу из строя либо конкретного узла (где было установлено кольцо), либо оборудования в целом. Для беспрерывной работы деталей оборудования необходимо своевременно приобрести и заменить кольца РТИ. В зависимости от сечения кольца (seal-ring) делятся на o-ring, x-ring.',
    features: ['Термостойкие', 'Резиновые', 'Полиуретановые', 'Полиамидные', 'Фторопластовые', 'Фторкаучуковые'],
    selected: false,
    show: false,
  ...
  },
  {
    ...
    description:
      'Уплотнение гидроцилиндра поршня - это манжета поршневая предназначенная для обеспечения герметичности в зазоре поршень-цилиндр. Отсутствие уплотнения поршня приводит к снижению коэффициента полезного действия двигателя, усилию гидропривода. С ростом зазора возрастает риск полного отказа оборудования. Уплотнение поршня имеет форму кольца. Оно фиксируется в открытых или закрытых проточенных канавках и состоит из одного или нескольких элементов. Изготавливается из фторопласта, полиуретана , резины (или с наполнителями).',
    selected: false,
    show: false,
...
`,
    features: ['Торцевые по кольцевой поверхности', 'Сальниковые', 'Манжетные', 'Разрезные пружинные кольца'],
    selected: false,
    show: false,
